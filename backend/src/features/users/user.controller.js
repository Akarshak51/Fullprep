import User from "./user.model.js";
import Submission from "../submissions/submission.model.js";
import Problem from "../problems/problem.model.js";
import { ApiError } from "../../shared/errors/ApiError.js";
import { successResponse } from "../../shared/utils/apiResponse.js";
import { toPublicUser } from "../../shared/utils/userProfile.js";

async function stats(userId) {
  const solved = await Submission.distinct("problemId", {
    userId,
    status: "Accepted",
  });
  const byDifficulty = await Problem.aggregate([
    { $match: { _id: { $in: solved } } },
    { $group: { _id: "$difficulty", count: { $sum: 1 } } },
  ]);
  const languages = await Submission.aggregate([
    { $match: { userId } },
    { $group: { _id: "$language", count: { $sum: 1 } } },
    { $sort: { count: -1 } },
  ]);

  return {
    problemsSolved: solved.length,
    totalProblems: await Problem.countDocuments(),
    solvedByDifficulty: Object.fromEntries(
      byDifficulty.map((x) => [x._id, x.count]),
    ),
    languagesUsed: languages,
  };
}

export async function getMe(req, res) {
  return successResponse(res, {
    user: toPublicUser(req.user),
    stats: await stats(req.user._id),
  });
}

export async function profile(req, res) {
  const user = await User.findOne({
    username: req.params.username.toLowerCase(),
  });

  if (!user) {
    throw new ApiError(404, "User not found");
  }

  const ownProfile =
    req.user && req.user._id.toString() === user._id.toString();

  // A user's own profile is always accessible.
  if (!ownProfile && user.settings?.privacy?.publicProfile === false) {
    throw new ApiError(403, "This profile is private");
  }

  const data = {
    ...toPublicUser(user),
    ...(await stats(user._id)),
  };

  if (ownProfile || user.settings?.privacy?.showSolvedProblems !== false) {
    data.solvedProblems = await Submission.find({
      userId: user._id,
      status: "Accepted",
    })
      .populate("problemId", "title slug difficulty")
      .sort({ createdAt: -1 })
      .limit(100);
  }

  data.bookmarks = await Problem.find({
    _id: { $in: user.bookmarks || [] },
  }).select("title slug difficulty");

  data.badges = user.badges || [];
  data.achievements = user.achievements || [];

  return successResponse(res, data);
}

export async function updateMe(req, res) {
  const allowed = ["name", "username", "bio", "avatar"];
  for (const key of Object.keys(req.body || {})) {
    if (!allowed.includes(key)) delete req.body[key];
  }

  Object.assign(req.user, req.body);
  await req.user.save();

  return successResponse(
    res,
    { user: toPublicUser(req.user) },
    "Profile updated",
  );
}
